<!-- Header -->

<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/news_ru.png" alt="DEM logo" width="800">

Проект пользовательского ремастера / патча для игры Ex Machina.

* Скачать моды можно на ![DEM logo](https://user-images.githubusercontent.com/79088546/174311991-c20e04bb-1cad-44e2-a0e9-5984de6d8d55.png) **[нашем форуме в разделе "Моды для Ex Machina"](https://forum.deuswiki.com/mods)**

* А так же на ![Discord logo](https://user-images.githubusercontent.com/79088546/174305727-755adfa0-57c2-41b0-9717-8476fcbc4567.png) [Discord сервере DEM](https://discord.gg/deus-ex-machina-522817939616038912) в разделе #витрина-модов


---
### ❄️ Мод-дайджест - Начало 2025 [2025.01.11]
<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/over_phys.png" alt="Logo" width="800">

С последнего дайджеста прошло прилично времени и произошло много всего интересного. Не теряя времени, начнём!

Мы поработали над нашими сайтами, [форум](https://forum.deuswiki.com) был полностью перезапущен как новая удобная площадка для публикации модов, обучения моддингу и общения моддеров и игроков, а наша [Деус Вики](https://deuswiki.com/w/Deus_Ex_Machina_%D0%92%D0%B8%D0%BA%D0%B8) с информацией про моддинг, так же стала доступна всембез VPN.

Новые релизы последних месяцев:
* **[OverPhys](https://forum.deuswiki.com/t/overphys/2931/1)** - меняет поведение и физику автомобилей, через комплексную настройку подвески, физики поверхностей. Так же имеет другие интересные трюки в рукаве, доступные для опциональной установки
* **[Haadotorakku no Mokushiroku](https://forum.deuswiki.com/t/haadotorakku-no-mokushiroku/2954/1)** - мемный релиз, машинно переводит игру на японский, с нейро-озвучкой и вниманием к деталям. Выбор виабу и треш-стримеров сообщества 🤡
* **Rusty Interface** - новый мини-мод от MrKarfen, с более потёртым и видавшим виды тейком на интерфейс КомРема

Новые альфа-бета-пре-релизы сезона:
* **kto Mod Remastered [Alpha version]** - ранняя версия порта именитого kto MOD на КомРем, контент пока портирован только частично
* Сделаем исключение и упомянем мод устанавливающийся без ComMod - **Меридиан 113: История Американского Героя** от Joomer, расширяющая сюжет М113, получила первый демо-релиз

Обновы существующих модов:
* **[Absque nota](https://forum.deuswiki.com/t/absque-nota/2967/1)** регулярно обновляется и добралась до версии Beta 0.8.2 Ещё больше классных тачек и других изменений!
* **[Improved Storyline](https://forum.deuswiki.com/t/improved-storyline/2948/4)** в 1.2.1 получил толстую пачку багфиксов и небольших геймплейных улучшений
* **[Different Things](https://forum.deuswiki.com/t/different-things/2938/1)** 1.3.3 получил совместимость с [Doomer Wave](https://forum.deuswiki.com/t/doomer-wave/2945) и раунд полировки
* **Искатели приключений** 0.4 обзавёлся новой, финальной главой, надеемся релиз уже скоро

🟠 **Скачать свежий ComMod 2.2.1** можно [на нашем Github](https://github.com/DeusExMachinaTeam/EM-CommunityPatch/releases), а моды как всегда можно найти [в Discord DEM](https://discord.gg/deus-ex-machina-522817939616038912) или **[на нашем новом форуме](https://forum.deuswiki.com/mods)**.



---
### 🌞 Мод-дайджест - Начало лета 2024 [2024.06.10]
<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/zil.png" alt="Logo" width="800">
Прошло несколько месяцев, надеемся все смелые уже успели попробовать Армаду и готовы к следующему приключению. Пока в сообществе наблюдается пауза и крупных новых релизов почти нет, мы решили заняться сезонным "обслуживанием" нашего парка модов - сделать старые моды доступнее, а мод менеджер удобнее.

Первый результат этой работы, это новый ["Гайд по модам" в нашем Discord (в форуме #гайды)](https://discord.gg/deus-ex-machina-522817939616038912). Там перечислены известные нам моды, которые можно установить с помощью ComMod. Включая специально перепакованные классические моды для чистой игры. Да, простая перепаковка под ComMod не сделала их совместимыми с КомРемастером, для этого нужно аккуратное ручное портирование, но, по крайней мере, установка через мод менеджер решит большинство проблем с запуском этих модов, применив к exe минимальные бинарные правки, ComPatch mini.

Перейдём к перечислению вышедших обнов.
Вскоре после релиза КомРема 1.14.1 подтянулись апдейты для популярных проектов:
* **ImprovedStoryline 1.2, Extra Machines 1.2** и **Doomer Wave 1.4.2** - в новых версиях получили перевод на английский и разнообразные багфиксы
* **Absque nota Beta 0.7.5** - мод на автомобили от Uno Regico ещё не вышел из бета статуса, а в этом апдейте добавил новые кабины с моделями повреждения и другие изменения
* **AmmoMiniMod Reborn** - вышедший ещё в конце января, это новая реализация старой идеи, добавить всем пушкам в игре ограниченные патроны. Совместим с кучей других модов на КомПатч и КомРем (Автор: MrKarfen)


Для чистой игры стали доступны:
* **ZiL Mod 1.1** - спустя больше трёх лет *Отдел «М»* возвращается с новой версией! Добавлены две новые единицы военной техники, 4ый уровень модификации для заглавного "Бизона" (Зил-130), парочка новых квестов и не только - всё это ждёт вас в новой версии.
* **kto MOD, nekto MOD, Жизнь Тракториста, Karl's Storyline, Leangle's New Storyline, Fall, Kardanich Interface Mod, ColorMaps, Constructor mod, Retank Mod (Lite), Kraycity, Booster mod, EMR Mod, Exclusive Mod, Vanila 404 FE, ЖАГ, Могучая Держава и Последний Завет** - были адаптированны для установки с помощью ComMod и доступны в "Гайде по модам" и теме "Архив модов" в нашей [**#витрине-модов**](https://discord.gg/deus-ex-machina-522817939616038912)

Чтобы поддержать сложные опции установки некоторых классических модов (например Fall и его поражающий воображение выбор опционального контента), был обновлён и сам мод менеджер, получив версию 2.2. Новая версия быстрее запускается, меньше весит, не лагает даже с 50+ модами в библиотеке и в целом заметно поумнела и пошустрела. Добавился нативный релиз для Linux, и был собран совместимый с Windows 7 релиз ранней консольной версии ComMod, так что теперь почти любой желающий сможет попробовать улучшения КомРема на своём ПК, независимо от выбора ОС. Новый, удобный показ промо-скриншотов модов стал ещё одним улучшением.

🟠 **Скачать ComMod 2.2** можно [на нашем Github](https://github.com/DeusExMachinaTeam/EM-CommunityPatch/releases), а моды как всегда можно найти [в Discord DEM](https://discord.gg/deus-ex-machina-522817939616038912).


---
### 🛳️ Вышел мод Armada - мод про корабли для Ex Machina [2024.04.04]
<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/armada.png" alt="Logo" width="800">
Сегодня вышел наверное "самый крутой из странных" и "самый странный из крутых" модов на махину - ARMADA.
Или как скромно, но точно иногда описывает его автор: "мод с кораблями".
Внизу напоминаю как выглядел трейлер этого, по сути, первого мода который можно назвать тотал-конвершном на базе движка Махины.

Я не знаю, что тут ещё можно комментировать? В это надо просто играть.

Внимание: **этому моду потребуются для установки новейшие ComMod 2.1 и ComRem 1.14.1**, ссылки можно найти в новости ниже!

<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/armada_screen.webp" alt="Screen" width="800">

🛳️ **Скачать Армаду** можно [на Github](https://github.com/lyokhatankist/ExM-Armada/releases) и [на ModDB](https://www.moddb.com/mods/hta-armada/downloads/armada).
🔴 [Смотреть трейлер Armada на YouTube](https://www.youtube.com/watch?v=CaSsOAplJw8) 

Авторы: **DOLBOYASCHERS TEAM**

---
### 🔥 Вышло обновление Комрема - 1.14.1, вышел новый ComMod 2.1! [2024.04.03]

Вышла новая версия Community Remaster, это первый официальный релиз КомРема и КомПатча с поддержкой английского!
Помимо этого был поправлен небольшой баг добавленный в прошлой версии.

Сам КомРем в этом релизе был адаптирован под новейшую версию ComMod 2.1, который содержит огромное количество внутренних улучшений и сильно повышает удобство работы для моддеров. Например, появилась возможность распространять разные варианты своего мода одним архивом без дупликации файлов.
Для игроков же этот релиз важен тем что развязывает наши руки для работы над следующей новой полноценной версией мода. Следите за новостями!

❗Установить новый КомРем получится только с помощью ComMod 2.1, новые версии модов так же будут часть использовать его фичи, так что обязательно обновитесь до новейшей версии!

Скачать можно на нашем Github: [CommunityRemaster 1.14.1](https://github.com/DeusExMachinaTeam/EM-CommunityPatch)

---
### 🔧 Вышел мод stranded - новый выживач в Ex Machina [2023.12.27]
<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/stranded.png" alt="Logo" width="800">

🔸**stranded** - концептуальная модификация для Ex Machina Community Remaster, которая расположилась на стыке жанров. В stranded присутствуют элементы выживания, головоломки и визуальной новеллы без... новеллы? Герою модификации предстоит решить загадки острова Сакадин, чтобы вернуть себе память и найти выход. 

<img src="https://raw.githubusercontent.com/DeusExMachinaTeam/ComModNews/main/assets/stranded_screen.webp" alt="Screen" width="800">

Мод можно найти [в нашем Discord](https://discord.gg/deus-ex-machina-522817939616038912) или [GitHub мода](https://github.com/Gnome627/stranded).

Автор: **Gnome627**

---
### 🍂 Мод-дайджест - Август-Декабрь 2023 [2023.12.11]

Осенний дайджест запоздал не просто так, а чтобы захватить день рождения игры... ну или по крайней мере это будет официальным оправданием.

8 декабря игре исполнилось 18 лет! С днём рождения, Ex Machina! Больше интересностей в связи с ДР можно найти в канале **#новости-сообщества** [в нашем Discord](https://discord.gg/deus-ex-machina-522817939616038912), ну а тут сосредоточимся на модах.

В сообществе варятся несколько новых проектов, пожелаем им гладкой разработки и скорейшего релиза, а пока пройдёмся по небольшому списку обновлений модов совместимых с Community Remaster 1.14:
* **Extra Machines 1.0.5** получил версию совместимую с актуальным ISL 1.1! Для любителей сборок - отличный вариант: ComRem+ISL+ExtraMachines - обновленная игра, сюжет и пак крутых новых тачек 👍
* **Different Things 1.3.0**, другой контент мод на игру, обновился, прибавив ещё немного нового и улучшая баланс своего контента (совместим так же и с ISL 1.1)
* **Честь наёмника 1.1.0** - мод, позволяющий пройти сюжет демо-версии Ex Machina, обновился до нового Комрема и пофиксил некоторые баги
* **Doomer Wave 1.4** в новой версии ч/б мод стал ещё чуточку более стильным и депрессивным, обновив как визуал, так и музыкальное сопровождение

Так же парочка новых релизов:
* **Ex Machina: Upgrade** - в конце сентября вышла первая публичная версия сюжетно-геймплейного мод на игру от моддера Carsen, создающего в нём свой вариант переосмысления сюжета оригинальной игры. Мод находится на ранних этапах разработки
* **Textures Updated** - в последний день лета вышел небольшой контент мод от MrKarfen, среди его фишек: перевод некоторых текстур с текстом на русский и правки текстур машин в авторском стиле

Сам мод-менеджер **ComMod** тоже обновился до версии 2.0.10, исправляя по пути некоторые баги. Пользователям 2.0.6 стоит [обновиться](https://github.com/DeusExMachinaTeam/EM-CommunityPatch/releases/tag/v1.14-230616a), это избавит от возможных проблем в базовых функциях и при установке модов.


---
### ☀️ Мод-дайджест - Июнь-Июль 2023 [2023.07.26]

После релиза ComRem 1.14, некоторые авторы оперативно обновили свои моды для совместимости с ним и ComMod 2.0!
* **ISL 1.1**, новая версия лучшего сюжетного мода для игры.
  ISL это знак качества и не нуждается в дополнительных рекомендациях. В одной из последних версий были доработаны и улучшены практически все катсцены мода, а в 1.1 помимо полной совместимости со свежим ComPatch и ComRem, автор подготовил немало других изменений и багфиксов.
* **Extra Machines 1.0.5**, свежий билд крутого мода добавляющего в игру новые играбельные машины.
* **DoomerWave 1.3.6**, обновлённая версия атмосферного депрессивного "фильтра" на оригинальную игру. В новой версии: доработки визуала, опциональный текстурный пак.

Так же появился новый микро-мод:
* "Плавные переходы между локациями", микро-мод с несомненно звучным именем. Ну, зато можно сразу догадаться что он делает

Как всегда, скачать все моды можно на [Discord сервере DEM](https://discord.gg/deus-ex-machina-522817939616038912) в разделе **#витрина-модов**

---
### 🔥 Вышла новая версия Комрема - 1.14! [2023.06.16]

Вышла новая версия Community Remaster! Главные фичи:
* исправлены фризы при спавне машин, включая машины противника
* увеличена максимальная дальность отрисовки в игре в ~2 раза
* новая версия мод менеджера ComMod с графическим интерфейсом, в нём вы читаете эту новость ❤️
* а так же другие исправления и улучшения игры!

Скачать можно на нашем Github: [CommunityRemaster 1.14](https://github.com/DeusExMachinaTeam/EM-CommunityPatch)

---
### 🔧 Вышел мод Extra Machines на ComRem [2023.05.26]

**Extra Machines** - аддон ассет-пак, добавляющий новые машины в оригинальную Ex Machina.
Все машины в паке были созданы специально для Ex Machina, а не являются портами из других игр, как в многих других модах. Мод является беспрецедентным по уровню качества добавляемых машин, разнообразию доступных кабин и кузовов, наличию модели повреждений у всех играбельных машин.

Автор моделей и настройки на ванильную игру: **Spok**.

Автор порта на ComRem: **Overline**

---
### 🔧 Вышел мод Честь Наёмника - порт сюжета демо-версии на ComRem [2023.05.24]

**Честь Наёмника** - первый мод из цикла Old Stories.
Данная модификация является переносом сюжета из Demo-версии ExMachina с исправлениями и некоторыми нововведениями.

Автор: **MrKarfen**

---
### 🔧 Вышла новая версия Improved Storyline для ComRem [2023.05.18]

**Improved Storyline** - сюжетная модификация к игре Ex Machina, главная цель которой - переосмыслить и разнообразить сюжет оригинальной игры.
В этой модификации вас ждут переработанные диалоги, более полно раскрывающие персонажей и саму вселенную игры, новые сюжетные повороты, более разнообразные концовки и другие качественные улучшения истории.

Автор: **Павлик PRG**

---
### 🔧 День рождения EM2ch и новый мод в разработке - EM: Armada [2023.04.24]

EM2ch, наши друзья и соратники по разработке ComPatch/ComRemaster вчера отпраздновали своё семилетие.
По хорошей традиции Петрович выложил в этот день не только поздравительный пост, но и что-то новенькое, анонс нового мода от прославленной DOLBOYASCHERS TEAM. Насладиться широтой полёта мысли можно в трейлере на Ютубе, а мы немного запоздало присоединяемся к поздравлением и желаем республике ещё много продуктивных и творческих лет.

[Смотреть трейлер на YouTube](https://www.youtube.com/watch?v=CaSsOAplJw8)

---
### 🔧 Вышел мод Different Things на ComRem [2023.02.25]

Different Things - небольшой мод, добавляющий в игру автомобиль "Бульдог" и восемь новых образцов оружия. Помимо этого данный мод вносит небольшие правки в баланс и исправляет некоторые ошибки.

Автор: **MrKarfen**

Использован контент от авторов: **Seel, AndrewN**

(Доступны версии для ComRem, ISL, DoomerWave, ExtraMachines)

---
### 🔧 Вышла новая версия Doomer Wave на ComRem [2023.01.29]

Вышло обновление 1.3
Doomer Wave - модификация, направленная на изменение игровой атмосферы в депрессивную сторону. Данная модификация не добавляет новый сюжет, он остаётся оригинальным.
На 99% игра теперь в чёрно-белом формате, переработаны пост-эффекты погоды, переработан оригинальный саундтрек (slowed+reverb) и не только.

Автор: **СТАКАН**

---
### 🔥 Вышла новая версия Комрема - 1.13! [2022.12.31]

К этому празднику мы подготовили для вас небольшой подарок - новый пререлиз Комрема под версией 1.13 (и конечно Компатча тоже). В нём вы найдёте некоторое количество улучшений и восстановления контента игры, новый менеджер модов и установщик Комрема по совместительству - ComMod (DEM Community Mod Manager), ну и чтобы вы наконец поняли во что играете - полный список всех изменений в Компатче и Комреме: [github.com/DeusExMachinaTeam/EM-CommunityPatch](https://github.com/DeusExMachinaTeam/EM-CommunityPatch)
